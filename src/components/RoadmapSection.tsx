  ...
  return (
    <section id="roadmap" className="py-14 relative overflow-hidden dark" style={{ background: "hsl(158 40% 12%)" }}>
      ...
      <div className="container mx-auto px-4 relative z-10">
        ...
        {/* Cards grid */}
        <StaggerContainer className="max-w-5xl mx-auto grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-5" staggerDelay={0.12}>
          {milestones.map((m) => {
            const cfg = statusConfig[m.status as keyof typeof statusConfig];
            return (
              <StaggerItem key={m.phase}>
                <div
                  className={`${cfg.glow} rounded-2xl p-6 relative overflow-hidden flex flex-col gap-4 transition-all duration-300 hover:-translate-y-1 h-full`}
                >
                  <span
